...
class ExposedLocalidadeRepository(
    ...
    override suspend fun inicializar() {
        suspendTransaction(database) {
            SchemaUtils.create(PaisesTable, DivisoesTable, CidadesTable)
            SchemaUtils.createMissingTablesAndColumns(PaisesTable, DivisoesTable, CidadesTable)
            runCatching { exec("ALTER TABLE divisao ALTER COLUMN sigla DROP NOT NULL") }
        }
        seed()
    }
    ...
    override suspend fun listarDivisoes(idPais: Long): List<Divisao> = suspendTransaction(database) {
        DivisoesTable.selectAll()
            .where { (DivisoesTable.idPais eq idPais) and (DivisoesTable.status neq Status.DELETADO.name.lowercase()) }
