...
export default function MarcasPage({ navReset }: { navReset: number }) {
  const { t } = useI18n();
  ...
  async function salvar() {
    setErro(null);
    if (!nome.trim()) {
      setErro(t("marca.error.nameRequired"));
      return;
    }
    setSalvando(true);
    try {
      const body = { nome: toTitleCase(nome), status };
      if (editando) await atualizarMarca(editando.id, body);
      else await criarMarca(body);
      setFormAberto(false);
      await carregar();
    } catch (e) {
    ...
    return (
      <div className="space-y-5 max-w-xl">
        ...
        <h1 className="text-xl font-semibold" style={{ fontFamily: "var(--font-display)", color: v("--text") }}>
          {editando ? t("marca.edit") : t("marca.new")}
        </h1>
                ...
                <td className="drive-td text-right">
                  ...
                  <button className="text-xs cursor-pointer" style={{ color: "var(--danger)" }}
                    onClick={async () => {
                      ...
                      try { await excluirMarca(e.id); await carregar(); }
